...
class _LevelsPageState extends State<LevelsPage> {
                  ...
                  child: GestureDetector(
                    onTap: () {
                      if (i == 0 || levelCompletionStatus[i - 1]) {
                        ...
                        if (i == 0) {
                       Navigator.push(context, MaterialPageRoute(builder: (_) => const LevelOneScreen()));
                       } else if (i == 1) {
  Navigator.push(
    context,
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text('Level ${i + 1} is not implemented yet.')),
  );
}
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        border: Border.all(
                          color: levelCompletionStatus[i] ? Colors.greenAccent : Colors.white,
                          width: 2,
                        ),
                      ),
                      child: Center(
...
Future<void> openDoor() async {
  ...
  character.add(MoveEffect.by(Vector2(size.x * 0.1, 0), EffectController(duration: 1.0)));
...
class LevelTwo extends FlameGame  {
  ...
  void update(double dt) {
    ...
    if ((character.position.x - trap2.position.x).abs() < 80) {
      ...
      trap2.position.x = (trap2.position.x - moveSpeed * dt).clamp(targetX, trap2OriginalPos.x);
    } else {
      trap2.position.x = (trap2.position.x + moveSpeed * dt).clamp(trap2OriginalPos.x - trapMoveDistance, trap2OriginalPos.x);
    }
    ...
    if ((character.position.x - trap3.position.x).abs() < 80) {
      ...
      trap3.position.x = (trap3.position.x + moveSpeed * dt).clamp(trap3OriginalPos.x, targetX);
    } else {
      trap3.position.x = (trap3.position.x - moveSpeed * dt).clamp(trap3OriginalPos.x, trap3OriginalPos.x + trapMoveDistance);
    }
...
class levelFour extends FlameGame {
  ...
  Future<void> onLoad() async {
    ...
    character = SpriteAnimationComponent()
      ..animation = SpriteAnimation.spriteList(walkFrames, stepTime: 0.3, loop: true)
      ..size = Vector2(size.x * 0.06, size.x * 0.06)
      ..position = Vector2(size.x * 0.05, size.y * 0.65);
    add(character);
    ...
    door = SpriteAnimationComponent()
      ..animation = SpriteAnimation.spriteList(
        [Sprite(doorSheet, srcPosition: Vector2(0, 0), srcSize: Vector2(doorSheet.width.toDouble(), frameHeight))],
        stepTime: 0.1,
      )
      ..size = Vector2(size.x * 0.15, (size.x * 0.15) * (frameHeight / doorSheet.width))
...
class Levelfive extends FlameGame {
  ...
  Future<void> onLoad() async {
    ...
    character = SpriteAnimationComponent()
      ..animation = walkAnimation
      ..size = Vector2(size.x * 0.06, size.x * 0.06)
      ..position = Vector2(size.x * 0.4, size.y * 0.65); // between trap1 and trap2
    add(character);
    ...
    trap1 = SpriteComponent()
      ..sprite = trapSprite
      ..size = trapSize
      ..position = Vector2(size.x * 0.34, trapY); // closer to character (left side)
    add(trap1);
    ...
    final frameHeight = doorSheet.height / 10;
    door = SpriteComponent()
      ..sprite = Sprite(doorSheet, srcPosition: Vector2(0, 0), srcSize: Vector2(doorSheet.width.toDouble(), frameHeight))
      ..size = Vector2(size.x * 0.15, (size.x * 0.15) * (frameHeight / doorSheet.width))
  ...
  void die() {
    if (!isDead) {
      isDead = true;
      showLoseOverlay = true;
      character.add(OpacityEffect.to(0, EffectController(duration: 1.2, curve: Curves.easeOut)));
      onPlayerDied?.call();
    }
  }
  ...
  Future<void> openDoor() async {
    ...
    character.addAll([
      OpacityEffect.to(0, EffectController(duration: 0.7, curve: Curves.easeIn)),
      MoveEffect.by(Vector2(size.x * 0.1, 0), EffectController(duration: 1.0, curve: Curves.linear)),
    ]);
  ...
  void restart() {
    ...
    removeWhere((component) => component is SpriteAnimationComponent && component.animation == doorAnimation);
...
class LevelSix extends FlameGame {
  ...
  Future<void> onLoad() async {
    ...
    initialDoor = SpriteComponent()
      ..sprite = Sprite(doorSheet,
          srcPosition: Vector2(0, 0),
          srcSize: Vector2(doorSheet.width.toDouble(), frameHeight))
      ..size = Vector2(size.x * 0.15,
          (size.x * 0.15) * (frameHeight / doorSheet.width))
      ..position = Vector2(size.x * 0.8, size.y * 0.6);
    add(initialDoor);
  ...
  triggerTransition();
    ...
    // Chasing trap logic
    if (chasing && newChasingTrap != null) {
      final direction =
  ...
  if (doorOpened &&
    finalDoor != null &&
    character.position.x >= finalDoor!.position.x - character.size.x * 0.3 &&
...
newChasingTrap = SpriteComponent()
  ..sprite = trapSprite
  ..size = Vector2(size.x * 0.05, size.x * 0.05)
  ..position = Vector2(initialDoor.position.x + size.x * 0.1, trapY); // farther to right
add(newChasingTrap);
